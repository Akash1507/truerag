...
@pytest.mark.asyncio
async def test_scrubbed_text_passed_to_chunk_stub() -> None:
    ...
    assert mock_chunk.call_args[0][0] != raw.decode()
...
async def test_log_includes_required_fields_and_no_text_content() -> None:
    ...
    with (
        ...
        await run_ingestion_pipeline(payload, AsyncMock(), _make_settings())
